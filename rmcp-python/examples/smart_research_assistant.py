        ...
        results = [
            {
                ...
                "snippet": f"Research shows AI is transforming software development through "
                f"automated code generation, testing, and deployment processes. "
                f"Impact factor: {90 + i}%",
            }
...
class SmartResearchAssistant:
    ...
    async def setup_tools(self):
        ...
        async def fact_check(claim: str, sources: list[str] | None = None) -> dict[str, Any]:
        ...
        @self.app.tool(timeout_ms=30000, idempotency_key_generator=lambda args: f"report-{args['research_id']}")
        async def generate_research_report(
            research_id: str, search_results: list[dict], analyses: list[dict], fact_checks: list[dict], query: str
        ) -> dict[str, Any]:
        ...
        @self.app.tool(idempotency_key_generator=lambda args: f"save-{args['research_id']}")
    ...
    async def conduct_research(self, query: str, research_id: str | None = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            search_result = await self.app.call_tool(
                "web_search", {"query": query, "num_results": 6}, idempotency_key=f"search-{research_id}"
            )
            ...
            for i, analysis in enumerate(analyses[:2]):  # Check top 2 analyses
                try:
                    ...
                    fact_check_result = await self.app.call_tool(
                        "fact_check",
                        {"claim": claim, "sources": [s["url"] for s in search_data["results"][:3]]},
            ...
            save_result = await self.app.call_tool(
                "save_research", {"research_id": research_id, "report": report}, idempotency_key=f"save-{research_id}"
            )
...
async def main():
    ...
    # Create FastRMCP app
    research_app = FastRMCP(mock_mcp_session, config=config, name="SmartResearchAssistant")
    ...
    try:
        ...
        async with research_app:
            ...
            for step in assistant.research_sessions[result["research_id"]]["steps"]:
                print(
                    f"• {step['step']}: {step['attempts']} attempts, "
