...
このガイドでは、FastMCP-Txデコレータを使用して信頼性の高いAIエージェントを構築する方法を実演します。堅牢なエラーハンドリングと配信保証を必要とするマルチステップワークフローに焦点を当てています。
...
# リサーチアシスタントを作成
research_agent = FastMCP-Tx(mcp_session, config=config, name="スマートリサーチアシスタント")
```
